...
int lastSendProgressVal = -1;
HWND g_hwndProgress;
extra_parameters* g_pluginExtra;
int progressCallback = -1;
//开始下载线程
bool startDownLaodThread = false;
...
void onTimerEx() {
	//static TCHAR buf[1024] = TEXT("");
	////转换数据
	//wsprintf(buf, TEXT("%d"), fs > 0 && fs != NOT_AVAILABLE ? MulDiv(100, cnt, fs) : 0);
	////写入数据
	//setuservariable(INST_9, buf);

	if (!startDownLaodThread)
	{
		return;
	}
	...
	if (IsWindow(g_hwndProgress)) {
		...
		int bps = cnt / time_sofar;
		//已下载大小
		int downSize = cnt / 1024;
		//当前进度计算(百分比)
		int percentage = (fs > 0 && fs != NOT_AVAILABLE ? MulDiv(100, cnt, fs) : 0);
		//验证值发生变化
		if (percentage == lastSendProgressVal) {
			return;
		}
		//累计大小
		int sumSize = (fs != NOT_AVAILABLE ? fs / 1024 : 0);
		//下载速度 
		int downSpeed = (bps / 1024, ((bps * 10) / 1024) % 10);
		//剩余时间(秒)
		int remain = (cnt > 0 && fs != NOT_AVAILABLE) ? (MulDiv(time_sofar, fs, cnt) - time_sofar) : 0;
		//脚本读取为倒序,即读到的第1个值为percentage
		//剩余时间(秒)
		pushint(remain);
		//下载速度
		pushint(downSpeed);
		...
		//记录最后的百分比
		lastSendProgressVal = percentage;
	}
}
...
//	if(*szCaption == 0) lstrcpy(szCaption, PLUGIN_NAME);
	if (*szUserAgent == 0) {
		lstrcpy(szUserAgent, INETC_USERAGENT);
	}
	...
	if (hwndParent != NULL && (childwnd = FindWindowEx(hwndParent, NULL, TEXT("#32770"), NULL)) != NULL && !silent)
	{
		SetDlgItemText(childwnd, 1006, *szCaption ? szCaption : PLUGIN_NAME);
	}
	// or NSIS do this before .onInit?
	else 
	{
		InitCommonControls();
	} 
	...
	if((hDlg = CreateDialog(g_hInstance, MAKEINTRESOURCE(szBanner ? IDD_DIALOG2 : (popup ? IDD_DIALOG1 : IDD_DIALOG3)),
